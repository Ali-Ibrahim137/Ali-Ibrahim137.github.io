...
The title contains a lot of terms that should be explained separately before we start. Given an undirected graph $$G = (V, E)$$ we define the following terms:
 ...
 - ***Independent Set (IS) :*** Subset of nodes $$U \subseteq V$$ with the following property:
...
A graph is called bipartite if we can split the set of nodes $$V$$ into two nonempty sets $$L, R$$ and the following two conditions are true:
...
A vertex cover is *minimum* if no other vertex cover has fewer vertices.
...
### Motivation problem
A company has $$m$$ different vacant jobs, $$n$$ employees applied for these jobs, and each employee applied for $$k_i$$ jobs. Each employee can take exactly one job, and each job has only one vacancy. Find a way to cover the most jobs.

#### Building the bipartite graph
We will build a bipartite graph $$G = (V, E)$$ with $$n+m$$ vertices and $$\sum_{i=1}^{n} k_i$$ edges. We add an edge between employee $$i$$ and job $$j$$ if the $$i_{th}$$ employee applied for the $$j_{th}$$ job. It's clear that this graph is bipartite as there won't be edges between any two jobs or any two employees.
...
Let's explain how this algorithm finds augmenting paths. We start with free vertex $$1$$. The algorithm will *wrongly* find augmenting path $$(1-3)$$, matching vertices $$1$$ and $$3$$.
...
We should also mention that the **MCBM** problem can be solved in $$O(\sqrt{V} * E)$$ using *Hopcroft Karp’s algorithm*. However I am not going to talk about it now; maybe in next articles I will talk about it in more detail.

## Back to the original problem
We have gone a little bit away from the original problem, started by the **MaxIS** problem and now we went through a lot of different topics. Actually you can't really understand how to find the **MaxIS** in a bipartite graph without going through all these steps. There are two extra things we should mention before finishing this article. First thing states that:

> The complement of a minimum vertex cover in any graph is the maximum independent set.

In conclusion we can say, in bipartite graphs solving the **NP-Hard MaxIS** can be reduced to solving the **NP-Hard MVC** problem. This can also be reduced to solving the **polynomial MCBM** problem.
...
Let's prove that all visited nodes of the right side are part of the **MVC**. When we start from an unmatched node from the left side, we can only go by a *free* edge to the right side. This node from the right side is clearly incident to some edge $$e \in M$$ (otherwise we would have matched these two nodes together). As the unmatched node from the left side is not part of the **MVC** then we must take the right side node. Next we go back to the left side using a *matched* edge. As the right side of this edge is part of the **MVC**, we can't take this node (check the previous paragraph). And so on until we finish traversing the graph.
...
We have proven that $$V_R$$ are definitely part of the **MVC** and $$V_L$$ are definitely not part of the **MVC**. For the $$U_L$$ and $$U_R$$ it turns out that all the $$U_L$$ nodes are definitely part of the **MVC** and $$U_R$$ are definitely not part of the **MVC**. To prove this, just consider the left side as right side, and the right side as left side for the unvisited nodes, and do the same as before.
